...
class Planet
  ...
  def get_planet_info
    rows = []
    rows << ['Name', @name]
    rows << ['Diameter (miles)', @diameter_in_miles]
    rows << ['Number of moons', @number_moons]
    rows << ['Has water', @has_water]
    rows << ['Mass (in kg)', @mass_in_kg]
    rows << ['Number of rings', @rings]
    rows << ['Solar orbit (earth days)', @solar_orbit]
    rows << ['Distance from the sun (miles)', @distance_from_the_sun]
    table = Terminal::Table.new :rows => rows, :title => "Planet Info"
